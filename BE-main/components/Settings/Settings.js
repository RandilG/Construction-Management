...
const getUserSettings = async function(req, res) {
  try {
    ...
    const sql = 'SELECT settings FROM homebuild.user_settings WHERE email = ?';
...
const updateSettings = async function(req, res) {
  try {
    ...
    const checkSql = 'SELECT id FROM homebuild.user_settings WHERE email = ?';
    ...
    connection.query(checkSql, [userEmail], (checkErr, checkResults) => {
      ...
      if (checkResults.length === 0) {
        // Insert new settings
        sql = 'INSERT INTO homebuild.user_settings (email, settings) VALUES (?, ?)';
        params = [userEmail, settingsJson];
      } else {
        // Update existing settings
        sql = 'UPDATE homebuild.user_settings SET settings = ? WHERE email = ?';
